...
describe Datecalc do
  ...
  let(:birthday4) { described_class.new(5, 1, today_leap)}
  let(:today_leap) { Date.new(2020,01,06) }
  ...
  context "when birthday is next year and its a leap year" do
    it 'calculates number of days until birthday' do
      expect(birthday4.days_until_birthday).to eq(365)
    end
  end
end
